...
if !exists('g:WintabsHiColor')
  let g:WintabsHiColor = {}
endif

for [key, val] in items(s:file_extension_colors)
  if !has_key(g:WintabsHiColor, key)
    let g:WintabsHiColor[key] = val
  endif
endfor

for [key, val] in items(g:WintabsHiColor)
  let icon_identifier = 'wintabsIcon_'.key
  ...
  if exists('g:WebDevIconsUnicodeDecorateFileNodesExtensionSymbols["'.key.'"]')
    ...
    exec 'hi def link '.icon_identifier.' WintabsNumCurrent'
  endif
